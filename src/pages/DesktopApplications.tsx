...
const DesktopApplications = () => {
    ...
    return (
        <>
        <Header heading='Desktop Application' />
        ...
        <Page className='px-12'>
            <div className="flex flex-col mt-8 md:mt-14 gap-y-2">
                {desktopApplications.map((application, index) => (
                    <ProductCard1 description1={application.description1} description2={application.description2} span={application.span} key={index} directionClass={`${index % 2 != 0 ? 'md:flex-row-reverse' : ''}`} img={application.img} heading={application.heading} />
                ))}
            </div>
        </Page>
        </>
